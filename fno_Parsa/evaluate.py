...
def evaluate_model(model, test_loader, target_variable=["Temperature"], stations=None, depths=None, work_dir=Path(".")):
    ...
    with torch.no_grad():
        for x, ys, mask in test_loader:  ##Changed
            ...
            mask = torch.flatten(mask, start_dim = 2).cpu().numpy()        # [num_nodes]
            y_true = torch.flatten(ys, start_dim = 2).cpu().numpy()         # [num_nodes]
            y_pred = torch.flatten(out, start_dim = 2).cpu().numpy()            # [num_nodes]

            # Fill with NaN where mask is False
            y_true_filled = np.where(mask, y_true, np.nan)
            y_pred_filled = np.where(mask, y_pred, np.nan)

            ys_true_full.append(y_true_filled[0])
            ys_pred_full.append(y_pred_filled[0])
    ...
    ys_true_denorm = np.concatenate([denormalize_variable(trgt, ys_true_full[:,ind:ind+1], scale_params_target)for ind, trgt in enumerate(target_variable)], axis = 1)
    
    for ind, trgt in enumerate(target_variable):
        # Flatten only valid values for metrics
        valid_mask = ~np.isnan(ys_true_denorm[:,ind:ind+1])
        ys_true_valid = ys_true_denorm[:,ind:ind+1][valid_mask]
        ys_pred_valid = ys_pred_denorm[:,ind:ind+1][valid_mask]
        ...
        print(f"RMSE: {rmse:.4f} {units[trgt]}")
        print(f"MAE : {mae:.4f} {units[trgt]}")
        print(f"R²  : {r2:.4f}")
    ...
    # 1. Scatter
        ...
        plt.title(f"Scatter: RMSE={rmse:.3f} {units[trgt]}, R²={r2:.3f}")
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(plot_dir / f"scatter_pred_vs_obs_{trgt}.png", dpi=150)
        ...
        plt.xlabel(f"Residual [{units[trgt]}]")
        plt.ylabel("Count")
        plt.tight_layout()
        plt.savefig(plot_dir / f"residual_hist_{trgt}.png", dpi=150)

        # 3. Depth–Station heatmap of mean residuals
        mean_resid = np.nanmean(ys_pred_full[:,ind] - ys_true_full[:,ind], axis=0)  # mean across time
        resid_2d = mean_resid.reshape(num_depths, num_stations)
        resid_2d_plot = resid_2d.T # shape: (num_depths, num_stations)

        plt.figure(figsize=(6, 4))
        plt.imshow(resid_2d_plot, cmap='coolwarm', aspect='auto', origin='lower')
        plt.colorbar(label=f"Residual (Pred - Obs) [{units[trgt]}]")
        ...
        if stations is not None:
            ...
            plt.yticks(range(num_stations), stations, rotation=45)
            plt.ylabel('stations')
        plt.title("Mean Residual by Station/Depth")
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig(plot_dir / f"residual_heatmap_{trgt}.png", dpi=150)
