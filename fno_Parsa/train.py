...
def train_snapshot_model(model, train_loader, val_loader=None, lr=1e-3, wd=1e-5, epochs=200, reduction = 'mean_snap',early_stopping_buffer = None, save_path=None, min_lr = 0):
    ...
    for ep in range(1, epochs + 1):
        ...
        if val_loader is not None:
            ...
            elif early_stopping_buffer is not None:
                earlystopping_counter += 1
                if (earlystopping_counter >= early_stopping_buffer) and (ep >= 15 ):  # want to train for at least 20 epochs
                    print(
                        f"Stopping early --> epoch valodation score {val_mse} has not decreased over {early_stopping_buffer} epochs compared to best {best_val_mse} ")
                    break
...
def train_model(data_train, data_val,width = 20, num_layers = 1, modes1 = None, modes2 = None, batch_size = 2, n_epochs=200, lr=1e-3, wd=1e-5, reduction = 'mean_snap', early_stopping_buffer = None,save_path=None):  ##Changed
    ...
    model, train_losses, val_losses, best_val_mse = train_snapshot_model(model, train_loader, val_loader, epochs=n_epochs, lr = lr, wd = wd,  reduction = reduction,early_stopping_buffer = early_stopping_buffer, save_path=save_path)   ##Changed
